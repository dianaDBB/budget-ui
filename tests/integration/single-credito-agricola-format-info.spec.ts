...
test.describe('Format Info — Single File Conversion', () => {
  ...
  test('should open the format popover for Crédito Agrícola when clicking the info button', async ({ budgetPage }) => {
    await test.step('Navigate to the Budget UI application', async () => {
      await budgetPage.goTo();
    });

    await test.step('Click the info icon button on the Crédito Agrícola card', async () => {
      await budgetPage.locators.singleSection.creditoAgricola.formatInfoButton().click();
    });

    await test.step('Verify the format popover header is visible and contains the bank name', async () => {
      await expect(budgetPage.locators.singleSection.formatInfoPopover.header(BankId.creditoAgricola)).toBeVisible();
    });
